...
if query:
    ...
    if selected_paper != "All papers":
        search_payload["filter"] = {"@eq": {"file_name": selected_paper}}
    ...
    with st.spinner("Searching cardiology research papers..."):
        result_df = session.sql(
            ...
            params=[json.dumps(search_payload)]
        ).to_pandas()
    ...
    if not results:
        ...
        if enable_llm:
            ...
            prompt = (
                "You are a cardiology research assistant. Answer the question below using ONLY the context provided. "
